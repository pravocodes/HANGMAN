...
def easyanimal():
    os.startfile(r'animal_easy.py')
    level.destroy()
def mediumanimal():
    os.startfile(r'animal_medium.py')
    level.destroy()
def hardanimal():
    os.startfile(r'animal_hard.py')
    level.destroy()
def easyinfo():
...
Button(level, text = 'Easy', image = photo1,command=lambda:easyanimal()).place(x=90,y=120)
Button(level, text = 'Medium', image = photo2,command=lambda:mediumanimal()).place(x=80,y=200)
Button(level, text = 'Hard', image = photo3,command=lambda:hardanimal()).place(x=90,y=280)
